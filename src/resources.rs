...
use crate::webgpu::{model, texture};
...
pub async fn load_string(file_path: &Path) -> anyhow::Result<String> {
    cfg_if! {
...
pub async fn load_binary(file_path: &Path) -> anyhow::Result<Vec<u8>> {
    cfg_if! {
        if #[cfg(target_arch = "wasm32")] {
            let url = format_url(file_name);
...
pub async fn load_texture(
    ...
    texture::Texture::from_bytes(device, queue, &data, Some(&file_path.display().to_string()))
}
...
pub async fn load_model(
    ...
    let (models, obj_materials) = tobj::load_obj_buf_async(
        ...
        |p| async move {
            let mat_text = load_string(&file_path.parent().unwrap().join(Path::new(&p)))
                .await
                .unwrap();
    ...
    let meshes = models
        ...
        .map(|m| {
            ...
            model::Mesh {
                name: file_path.to_str().unwrap().to_owned(),
                vertex_buffer,
                index_buffer,
